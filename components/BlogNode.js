            ...
            <Link id={post} href={'/blog/'+ post}>
                <div className="my-4 w-full px-8 py-4 mx-2 bg-white cursor-pointer hover:bg-gray-200 rounded-lg dark:bg-gray-800">
                    ...
                    <div className="my-2">
                        ...
                        <p className="font-jost mt-2 text-gray-600 dark:text-gray-300">{data?.description}</p>
                    </div>
                    
                    <div className="flex items-center justify-between">
                        <div className='flex'>
                            {!data?.tags ? null : data.tags.map(tag => <a href='#' key={tag} className="font-jost mr-2 px-3 py-1 text-sm font-bold text-gray-100 transition-colors duration-200 transform bg-gray-600 rounded cursor-pointer">{tag}</a>)}
                        </div>
