...
class Navigation extends React.Component {
  ...
  navGenerator = (name) => {
    return (
      <h2
        key={name}
        className="hidden font-prozaLibre lg:block px-2 py-1 mx-2 mt-2 text-lg font-medium text-gray-700 transition-colors duration-200 transform rounded-md md:mt-0 hover:bg-gray-300 "
      >
      ...
      <div key={name}>
        <Link href={`/${name === "Blog" ? "" : name.toLowerCase()}`}>
          <h2
            ...
            } cursor-pointer font-prozaLibre text-lg lg:hidden px-2 py-2 my-2 font-medium text-gray-700 transition-colors duration-200 transform rounded-md hover:bg-gray-300`}
          >
            {name}
          </h2>
        </Link>
