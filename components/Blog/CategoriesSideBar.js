...
const CategoriesSideBar = ({ categories }) => {
            ...
            <div key={tag}>
                <Link href={`/category/${tag}`}>
                    <div className="flex items-center my-2 mt-6  bg-white cursor-pointer hover:bg-gray-200 rounded-lg text-gray-700 hover:text-blue-600">
                        ...
                        <h2 className="font-hindSiliguri px-2 text-lg">
                            {tag}
                        </h2>
                    </div>
                </Link>
    ...
    return (
        <div
            ...
            className="block sticky top-14 overflow-hidden"
        ...
        >
            <div className="mt-12 mx-6 mb-6 2xl:ml-3 mr-4 md:mr-6 xl:mr-8 shadow-xl">
                <div className="py-4 2xl:w-64 bg-white rounded-lg shadow-lg">
                    <h1 className="font-prozaLibre mx-10 lg:mt-10 text-2xl lg:text-3xl font-semibold text-gray-800">
                        Categories
                    </h1>
                    <div className="flex flex-wrap 2xl:flex-col px-6">
                        {!categories
                            ? null
                            : categories.map((tag) => CategoriesGenerator(tag))}
                    </div>
                </div>
            </div>
        </div>
